                    ...
                    <div class="card-body">
                        <form action="{{ route('genres.update', $genre->id) }}" method="POST">
                            ...
                            <div class="form-group mb-3">
                                ...
                                <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title" value="{{ old('title', $genre->title) }}" required>
                                @error('title')
                            ...
                            <div class="form-group mb-3">
                                ...
                                <select class="form-control @error('status') is-invalid @enderror" id="status" name="status" required>
                                    <option value="1" {{ old('status', $genre->status) == '1' ? 'selected' : '' }}>Hoạt động</option>
                                    <option value="0" {{ old('status', $genre->status) == '0' ? 'selected' : '' }}>Không hoạt động</option>
                                </select>
                                @error('status')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
